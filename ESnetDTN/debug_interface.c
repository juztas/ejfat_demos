/**
 * Standalone program to debug interface detection for a given destination IP
 *
 * Compile: gcc -o debug_interface debug_interface.c
 * Usage: ./debug_interface <destination_ip>
 * Example: ./debug_interface 192.168.1.100
 */
...
#ifdef __linux__
/**
 * Get the outgoing interface using Netlink (Linux only)
 */
int get_interface_via_netlink(const char *dest_ip, char *ifname_out, int *mtu_out) {
...
/**
 * Get interface via connect() method (works on macOS and Linux)
 */
int get_interface_via_connect(const char *dest_ip, int dest_port, char *ifname_out, char *local_ip_out) {
    int sock;
    ...
    inet_ntop(AF_INET, &local_addr.sin_addr, local_ip_out, INET_ADDRSTRLEN);
...
/**
 * Test binding to INADDR_ANY
 */
void test_bind_any(const char *dest_ip, int dest_port) {
    int sock;
    struct sockaddr_in local_addr, dest_addr, bound_addr;
    socklen_t addr_len = sizeof(bound_addr);

    printf("\n--- Testing bind to INADDR_ANY ---\n");
    ...
    // Bind to INADDR_ANY (like E2SAR does)
    memset(&local_addr, 0, sizeof(local_addr));
    local_addr.sin_family = AF_INET;
    local_addr.sin_addr.s_addr = INADDR_ANY;
    local_addr.sin_port = 0;  // Let kernel choose port
    ...
    inet_ntop(AF_INET, &bound_addr.sin_addr, local_ip, INET_ADDRSTRLEN);

    printf("Bound to: %s:%d\n", local_ip, ntohs(bound_addr.sin_port));
...
int main(int argc, char *argv[]) {
    ...
    printf("Debugging interface selection for destination: %s:%d\n", dest_ip, dest_port);
    printf("========================================================\n\n");
    ...
    if (get_interface_via_netlink(dest_ip, ifname_netlink, &mtu) == 0) {
        printf("Interface: %s\n", ifname_netlink);
        printf("MTU: %d\n", mtu);
    } else {
    ...
    if (get_interface_via_connect(dest_ip, dest_port, ifname_connect, local_ip) == 0) {
        printf("Interface: %s\n", ifname_connect);
        printf("Local IP: %s\n", local_ip);
    } else {
    ...
    // Method 3: Test INADDR_ANY behavior
    test_bind_any(dest_ip, dest_port);
