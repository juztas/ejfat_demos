...
def process_file(stxm_file, output_filename, config, default_mask=None):
    ...
    metadata["geometry"]["distance"] = config["detector_distance"]
...
def preprocess(args):
    ...
    filename = args["path"].replace(".cxi",".stxm")
...
#    for stxm_filename in filename: #args.stxm_file:
...
#                expanded_stxm_filenames.append(filename)
    ...
    output_filename = '.'.join(filename.split('.')[:-1])+'.cxi'
    if not config['succinct']:
    ...
    with h5py.File(filename, 'r') as stxm_file:
        process_file(stxm_file, output_filename, config,
                     default_mask=mask)

    return #"Preprocessing complete. Starting reconstruction..."
