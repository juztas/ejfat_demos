...
import h5py
import glob
import image_handling, file_handling, config_handling

def process_file_std(stxm_filename, output_filename):
    ...
    with h5py.File(stxm_filename, 'r') as stxm_file:
        process_file(stxm_file, output_filename, config, default_mask=mask)
...
def process_file(stxm_file, output_filename, config, default_mask=None):
    ...
    translations = file_handling.read_translations_from_stxm(stxm_file)*config["translation_units"]
